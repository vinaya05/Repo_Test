...
CREATE PROCEDURE populate_liverail_revenue_master_fact_adsource()
BEGIN

DECLARE xfrom_table_name varchar(64) DEFAULT 'liverail_adsource_stage';
...
(
	SELECT * FROM (
			...
			from
			  liverail_adsource_stage
			INNER JOIN
			  liverail_partner_mapping
			ON
			  liverail_adsource_stage.partner_id = liverail_partner_mapping.partner_id
			INNER JOIN
			  v3.playlists_0
			ON
			  liverail_adsource_stage.playlist_id = playlists_0.id
			INNER JOIN
			  v3.wp_blogs
			ON
			...
			AND
			  liverail_adsource_stage.id > xhigh_water_mark
			AND
			  liverail_adsource_stage.id <= xnew_hwm
			GROUP BY
			  wp_blogs.account_id,
	...
	SELECT * FROM (
		SELECT
			...
			IF ( ( sum(revenue) / sum(impressions) ) * 1000 IS NOT NULL, ( sum(revenue) / sum(impressions) ) * 1000 , 0) as ecpm
		FROM
			t5m_stats.liverail_adsource_stage
		INNER JOIN
			t5m_stats.liverail_third_party_player_publisher lr
		ON
			  liverail_adsource_stage.partner_id = lr.partner_id
			AND
			  liverail_adsource_stage.id > xhigh_water_mark
			AND
			  liverail_adsource_stage.id <= xnew_hwm
...
SELECT max(id) INTO xnew_hwm FROM liverail_adsource_stage;
