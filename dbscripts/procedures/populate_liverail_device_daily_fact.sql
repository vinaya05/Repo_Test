...
CREATE PROCEDURE populate_liverail_device_daily_fact()
BEGIN

DECLARE xfrom_table_name varchar(64) DEFAULT 'liverail_master_fact';
DECLARE xto_table_name varchar(64) DEFAULT 'liverail_device_daily_fact';
DECLARE done INT DEFAULT 0;

-- DEMINSION and METRIC definitions
-- co_account_id	pub_account_id device_id day
...
REPEAT
  ...
  IF NOT done THEN
    ...
    SELECT count(1) INTO xcount FROM liverail_device_daily_fact
    WHERE 
    ...
    IF NOT xcount THEN
      INSERT INTO liverail_device_daily_fact (
        `co_account_id`,
        `pub_account_id`,
        `device_id`,
        `day`,
    ...
    ELSE
      ...
      DELETE FROM liverail_device_daily_fact
      WHERE
        co_account_id = xco_account_id and
        pub_account_id = xpub_account_id and
        device_id = xdevice_id and
