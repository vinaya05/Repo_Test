...
CREATE PROCEDURE populate_liverail_video_daily_fact()
BEGIN

DECLARE xfrom_table_name varchar(64) DEFAULT 'liverail_master_fact';
DECLARE xto_table_name varchar(64) DEFAULT 'liverail_video_daily_fact';
...
REPEAT
  ...
  IF NOT done THEN
    ...
    SELECT count(1) INTO xcount FROM liverail_video_daily_fact
    WHERE 
    ...
    IF NOT xcount THEN
      INSERT INTO liverail_video_daily_fact (
        `co_account_id`,
        `co_project_id`,
        `co_video_id`,
        `pub_account_id`,
    ...
    ELSE
      ...
      DELETE FROM liverail_video_daily_fact
      WHERE
        co_account_id = xco_account_id and
        co_project_id = xco_project_id and
        co_video_id = xco_video_id and
