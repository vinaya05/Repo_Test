...
CREATE PROCEDURE populate_generic_project_daily_fact()
BEGIN

-- Variables for cursor

DECLARE xfrom_table_name varchar(64) DEFAULT 'generic_master_fact';
DECLARE xto_table_name varchar(64) DEFAULT 'generic_project_daily_fact';
...
REPEAT
  ...
  IF NOT done THEN
    ...
    SELECT count(1) INTO xcount FROM generic_project_daily_fact
    WHERE 
    ...
    IF NOT xcount THEN
      INSERT INTO generic_project_daily_fact (
        `co_account_id`,
        `co_project_id`,
        `pub_account_id`,
	`content_type`,
    ...
    ELSE
      ...
      DELETE FROM generic_project_daily_fact
      WHERE
        co_account_id = xco_account_id and
        co_project_id = xco_project_id and
        pub_account_id = xpub_account_id and
