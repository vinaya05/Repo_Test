...
CREATE PROCEDURE populate_generic_account_daily_fact()
BEGIN

-- Variables for cursor

DECLARE xfrom_table_name varchar(64) DEFAULT 'generic_master_fact';
DECLARE xto_table_name varchar(64) DEFAULT 'generic_account_daily_fact';
...
REPEAT
  ...
  IF NOT done THEN
    ...
    SELECT count(1) INTO xcount FROM generic_account_daily_fact
    WHERE 
    ...
    IF NOT xcount THEN
      INSERT INTO generic_account_daily_fact (
        `co_account_id`,
        `pub_account_id`,
	`content_type`,
	`source_id`,
    ...
    ELSE
      ...
      DELETE FROM generic_account_daily_fact
      WHERE
        co_account_id = xco_account_id and
        pub_account_id = xpub_account_id and
	content_type = xcontent_type and
