...
CREATE PROCEDURE populate_generic_master_fact_from_revenue_master_fact()
BEGIN

-- Variables for cursor

DECLARE xfrom_table_name varchar(64) DEFAULT 'liverail_revenue_master_fact';
...
DECLARE xdevice_id smallint DEFAULT 1;
...
FROM
    t5m_stats.liverail_revenue_master_fact
WHERE
  t5m_stats.liverail_revenue_master_fact.id > xhigh_water_mark
AND
  t5m_stats.liverail_revenue_master_fact.id <= xnew_hwm
GROUP BY
    `co_account_id`,
    `co_project_id`,
    `co_video_id`,
...
SELECT max(id) INTO xnew_hwm FROM t5m_stats.liverail_revenue_master_fact;
